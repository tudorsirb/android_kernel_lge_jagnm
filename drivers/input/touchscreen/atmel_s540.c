...
#else
static struct mxt_data *touch_test_dev = NULL;
#endif
#ifdef MXT_GESTURE_RECOGNIZE
...
static void mxt_prepare_debug_data(struct mxt_data *data)
{
	...
	if (rawdata == NULL) {
		printk(KERN_ERR "%s() Fail to allocate sysfs data.\n",__func__);
		error = -ENOMEM;
		return ;
	}
	...
	if (!rawdata->reference) {
		printk(KERN_ERR "%s() Fail to alloc reference of rawdata\n", __func__);
		error = -ENOMEM;
	...
	if (!rawdata->delta) {
		printk(KERN_ERR "%s() Fail to alloc delta of fdata\n", __func__);
		error = -ENOMEM;
...
static int mxt_set_diagnostic_mode(struct mxt_data *data, u8 dbg_mode)
{
	...
	if (ret) {
		printk(KERN_ERR "%s() Failed change diagnositc mode to %d\n",
			 __func__,dbg_mode);
		goto out;
	}
	...
	if (dbg_mode & MXT_DIAG_MODE_MASK) {
		do {
			...
			if (ret) {
				printk(KERN_ERR "%s() Failed getting diagnositc mode\n", __func__);
				goto out;
			}
			msleep(20);
		} while (cur_mode != dbg_mode);
		printk("%s() current dianostic chip mode is %d\n", __func__, cur_mode);
	}
...
static int mxt_read_all_diagnostic_data(struct mxt_data *data, u8 dbg_mode, char *buf, int* len)
{
	...
	if (!dbg_object) {
		printk(KERN_INFO "%s() fail to get object_info\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	*len += snprintf(buf + *len, PAGE_SIZE - *len, "===============================================");
	...
	for (read_page = 0 ; read_page < end_page; read_page++) {
		...
		do {
			...
			if (ret) {
				printk(KERN_ERR "%s Read fail page\n", __func__);
				goto out;
			}
...
static void mxt_proc_t100_anti_message(struct mxt_data *data, u8 *message)
{
	...
	if (scr_status == 0 && num_rpt_touch == 0 && (data->anti->touch_area == 0 || data->anti->touch_area >= 0x30)) {
			if(data->anti->anti_area > 0 || data->anti->inter_area > 0){
				...
				dev_info(dev, "Anti #%d scr_status(%d) num_rpt_touch(%d) anti touch area(%d), touch area(%d), internal area(%d)\n",
				...
				if (data->anti->fcnt0_msg_cnt)
					dev_info(dev, "Reset Condition 1. Anti scr_status(%d) num_rpt_touch(%d) anti touch area(%d), touch area(%d), internal area(%d)\n",
						scr_status, num_rpt_touch, data->anti->anti_area, data->anti->touch_area, data->anti->inter_area);
				data->anti->fcnt0_msg_cnt = 0;
			}
	} else {
		if (data->anti->fcnt0_msg_cnt)
			dev_info(dev, "Anti scr_status(%d) num_rpt_touch(%d) anti touch area(%d), touch area(%d), internal area(%d)\n",
				scr_status, num_rpt_touch, data->anti->anti_area, data->anti->touch_area, data->anti->inter_area);
		data->anti->fcnt0_msg_cnt = 0;
	}
}
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (id < 0 || id >= MXT_MAX_NUM_TOUCHES) {
		dev_err(dev, "limited number of finger is %d\n", data->pdata->numtouch);
		return;
	}
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	if(data == NULL || data->object_table == NULL || data->info == NULL || data->raw_info_block == NULL)
		return -ENXIO;
  ...
  mutex_lock(&mxt_early_mutex);
	...
	for (i = 0; i < data->pdata->numtouch; i++) {
		...
		if (quick_cover_status || (use_quick_window && (cradle_smart_cover_status() == 1))){	/* drop touch event (out of quick window) */
			if(!((data->ts_data.curr_data[i].x_position > 380)
				&& (data->ts_data.curr_data[i].x_position < 1820)
				&& (data->ts_data.curr_data[i].y_position > 35)
				&& (data->ts_data.curr_data[i].y_position < 1860))){
...
static ssize_t mxt_fw_version_show(struct mxt_data *data, char *buf)
{
	...
	if (!mxt_mfts_for_mxt_start){
		ret = sprintf(buf, "%u.%u.%02X CRC:0x%06X\n",
				data->info->version >> 4, data->info->version & 0xf,data->info->build , MXT_LATEST_CONFIG_CRC_MFTS);	/* MFTS for 3.0 panel */
//		ret = sprintf(buf, "0x%06X\n",MXT_LATEST_CONFIG_CRC);	/* test mode */
	...
	return ret;
#endif
	return scnprintf(buf, PAGE_SIZE, "%u.%u.%02X patch_date:%d\n",
			 data->info->version >> 4, data->info->version & 0xf,
			 data->info->build, data->patch.date); 
}
...
static ssize_t mxt_selftest(struct mxt_data *data, char *buf, int len)
{
	...
	ret += sprintf(buf+ret, "====== MXT Self Test Info ======\n");
...
static void write_file(char *filename, char *data, int time)
{
	...
	printk(KERN_INFO "write open %s, fd : %d\n", (fd >= 0)? "success":"fail",fd);
	if (fd >= 0) {
		if (time > 0) {
			sys_write(fd, time_string, strlen(time_string));
			printk("Time write success.\n");
		}
...
static ssize_t mxt_firmware_update(struct mxt_data *data)
{
	...
	for(i=0;i<5;i++) {
		error = mxt_load_fw(dev, true);
		 if (error) {
			dev_err(dev, "The firmware update failed(%d) retry_cnt:%d\n", error, i);
		 	safety_reset(data);
		 }
		 else {
	...
	if(!error) {
		...
		for(i=0;i<5;i++) {
			error = mxt_read_info_block(data);
			if(error) {
				dev_err(dev, "read info block read failed(%d) retry_cnt:%d\n", error, i);
		 		safety_reset(data);
			}
			else {
				dev_info(dev, "read info block read succeeded\n");
				break;
			}
		}
    }
...
static ssize_t mxt_patch_debug_enable_store(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &i) == 1 && i < 2) {
		...
		dev_info(&data->client->dev, "patch debug enable write error\n");
		return -EINVAL;
	}
}
...
static ssize_t store_quick_cover_status(struct mxt_data *data, const char *buf, size_t size)
{
	...
	dev_info(&data->client->dev, "quick cover status = %s\n", (quick_cover_status == 1) ? "QUICK_COVER_ON(CLOSED)" : "QUICK_COVER_OFF(OPEN)");
	return size;
}
...
static ssize_t mxt_load_patch_from_ums(struct mxt_data *data, const char *buf, size_t size)
{
	...
	dev_info(dev, "%s ppatch:%p %p, date:%d\n", __func__, patch_data, data->patch.patch, data->patch.date);
...
static int mfts_fw_show(struct mxt_data *data)
{
	int ret = 0;
	...
	ret = __mxt_read_reg(data->client, 0x03, 1, &build);
	if(ret){
		dev_err(&data->client->dev, "Fail read Buid info\n");
		goto out;
	}
...
static int mfts_fw_ver_show(struct mxt_data *data)
{
	int ret = 0;
	u8 build = 0;	/* 3.0 - 170(AA) */
	u8 version = 0;
	...
	dev_info(&data->client->dev, "%s(), Show MFTS ver!\n",__func__);
	...
	//Read Version
	ret = __mxt_read_reg(data->client, 0x02, 1, &version);
	if(ret) {
		dev_err(&data->client->dev, "Fail read version info\n");
		ret = -1;
	}

	if (build == 0xAA && version == 0x30) {
	...
	dev_info(&data->client->dev, "0x%06X\n", data->config_crc);
	mxt_mfts = false;

	return ret;
}
static int mfts_show(struct mxt_data *data)
{
	int ret = 0;
	u8 build = 0;
	u8 version = 0;
	mxt_mfts = true;
	dev_info(&data->client->dev, "%s(), MFTS!\n",__func__);

	//Read Build ID
	ret = __mxt_read_reg(data->client, 0x03, 1, &build);
	if(ret) {
		dev_err(&data->client->dev, "Fail read Build info\n");
		ret = -1;
	}
	...
	if(build == 0xAA && version == 0x30) {
		...
		dev_info(&data->client->dev, "FW Version is 0x%x\n",build);
	}

	if (build == 0xAA && version == 0x30) {
		if(data->config_crc == MXT_LATEST_CONFIG_CRC_MFTS) {
			dev_info(&data->client->dev, "CRC is MXT_LATEST_CONFIG_CRC_MFTS\n");
		}else{
			dev_info(&data->client->dev, "CRC is not MXT_LATEST_CONFIG_CRC_MFTS\n");
		}
		ret = 0;
	} else {
		if(data->config_crc == 0) {
			dev_info(&data->client->dev, "CRC is ZERO\n");
		}else{
			dev_info(&data->client->dev, "CRC is not ZERO\n");
		}
		ret = build;
	}
...
static ssize_t mxt_mfts_enable_store(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if(data->mfts_enable) {
		...
		ret = __mxt_read_reg(data->client, 0x03, 1, &fw_build);
		...
		if(fw_build == MXT_LATEST_FW_BUILD && fw_version == MXT_LATEST_FW_VERSION) {
			printk("%s() IC F/W is same Img F/W\n",__func__);
		}
		else {
			ret = mfts_show(data);
			if(!ret) {
				ret = mfts_fw_show(data);
				ret = mfts_fw_ver_show(data);
				data->mfts_enable = 0;
			}
			else {
				printk("%s() Update Touch F/W to IC\n",__func__);
				...
				for(i=0;i<5;i++) {
					...
					error = mxt_initialize(data);
					if (error){
						printk("[lge_touch] %s() mfts resume mxt_initialize failed\n",__func__);
						mdelay(100);
						continue;
					}
					...
					if(data->info != NULL) {
						...
						printk("[lge_touch] %s() data->info = %s \n", __func__, data->info?"Exist":"NULL");
					}

					printk("[lge_touch] Need to firmware update? %d, %x!=%x, %x!=%x\n",
					...
					if (data->in_bootloader ||
						...
						printk("[lge_touch]Execute firmware update func\n");
						error = mxt_firmware_update(data);

						if (error) {
							safety_reset(data);
							printk(KERN_ERR "Failed to update firmware\n");
							continue;
						}
						else {
							break;
					...
					if (error) {
						safety_reset(data);
						printk(KERN_ERR "Failed to mxt_rest_init\n");
						msleep(100);
						continue;
					}
				}
...
static void change_ime_drumming_func(struct work_struct *work_ime_drumming)
{
	...
	dev_dbg(&data->client->dev, "change_ime_drumming_func. grip_suppresion_enable %d\n",grip_suppresion_enable);

	ret = mxt_write_reg(data->client, data->T40_address + 1, xlogrip);
	if (ret) {
		dev_err(&data->client->dev, "change_ime_drumming_func error. xlogrip(T40[1])\n");
	}
	dev_dbg(&data->client->dev, "change_ime_drumming_func. xlogrip %d\n",xlogrip);

	ret = mxt_write_reg(data->client, data->T40_address + 2, xhigrip);
	if (ret) {
		dev_err(&data->client->dev, "change_ime_drumming_func error. xhigrip(T40[2])\n");
	}
	dev_dbg(&data->client->dev, "change_ime_drumming_func. xhigrip %d\n",xhigrip);

	ret = mxt_write_reg(data->client, data->T40_address + 3, ylogrip);
	if (ret) {
		dev_err(&data->client->dev, "change_ime_drumming_func error. ylogrip(T40[3])\n");
	}
	dev_dbg(&data->client->dev, "change_ime_drumming_func. ylogrip %d\n", ylogrip);

	ret = mxt_write_reg(data->client, data->T40_address + 4, yhigrip);
	if (ret) {
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT) || defined(CONFIG_MACH_MSM8926_B2LN_KR)
		data->qwindow_size->x_min = (value * 15) / 10;
#else
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT) || defined(CONFIG_MACH_MSM8926_B2LN_KR)
		data->qwindow_size->x_max = (value * 15) / 10;
#else
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT) || defined(CONFIG_MACH_MSM8926_B2LN_KR)
		data->qwindow_size->y_min = (value * 15) / 10;
#else
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT) || defined(CONFIG_MACH_MSM8926_B2LN_KR)
		data->qwindow_size->y_max = (value * 15) / 10;
#else
...
static LGE_TOUCH_ATTR(patch_debug_enable, S_IWUSR | S_IRUSR, mxt_patch_debug_enable_show,
		   mxt_patch_debug_enable_store);
#ifdef MXT_GESTURE_RECOGNIZE
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	rc = of_property_read_u32(node, "atmel,mfts_used", &temp_val);
	if(rc){
		dev_err(dev, "Unable to read mfts_used\n");
		return rc;
	}else
		pdata->mfts_used = temp_val;
	/* config array size */
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && mxt_dev_data &&
		...
		if (*blank == FB_BLANK_UNBLANK) {
			if(NULL != &mxt_dev_data->client->dev)
